...
def merge_pr(pr_num, target_ref, title, body, pr_repo_desc):
    ...
    target_branch_name = (
        f"{TEMP_BRANCH_PREFIX}_MERGE_PR_{pr_num}_{target_ref.upper()}"
    )
    ...
    if len(commits) > 1:
        ...
        should_list_commits = result.lower().strip() == "y"
    else:
...
def get_remote_repos():
    ...
    return {repos[i]: repos[i+1] for i in range(0, len(repos), 3)}
...
def check_git_remote():
    ...
    name = f"{PROJECT_NAME}.git"
    for url in list(repos.values()):
        if not url.endswith(name):
            fail(f"Error: not a {PROJECT_NAME} git repo or at least one remote is invalid")

    if PR_REMOTE_NAME not in repos:
        fail(
            f"Error: PR_REMOTE_NAME ({PR_REMOTE_NAME}) environment variable has not been set!"
        )


    if PUSH_REMOTE_NAME not in repos:
        fail(
            f"Error: PUSH_REMOTE_NAME ({PUSH_REMOTE_NAME}) environment variable has not been set!"
        )
...
def main():
    ...
    pr_title = pr["title"]
    commit_title = input(f"Commit title [{pr_title}]: ")
    if commit_title == "":
